import type { FlexMatchupOut } from "@/types";

export default function FlexMatchupResultTable({ result }: { result: FlexMatchupOut }) {
  const { attacker_name, defender_name, attacker_points, weapons, total_expected_damage, total_models_killed, damage_per_point } = result;
    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-3">
        <div className="bg-blue-900/40 border border-blue-700/40 rounded-xl px-4 py-3">
          ...
          <div className="font-semibold text-white">{attacker_name}</div>
          <div className="text-xs text-gray-400">{attacker_points} pts</div>
        </div>
        ...
        <div className="bg-red-900/40 border border-red-700/40 rounded-xl px-4 py-3">
          ...
          <div className="font-semibold text-white">{defender_name}</div>
        </div>
      </div>
        ...
        <div className="bg-gray-800 rounded-xl overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {weapons.map((w) => (
                  <tr key={w.weapon_name} className="border-b border-gray-700/50 hover:bg-gray-700/30">
                ...
                <tr className="border-t-2 border-gray-600 font-semibold bg-gray-800">
                  <td className="px-4 py-3 text-gray-300" colSpan={2}>Total</td>
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className="bg-green-900/30 border border-green-700/40 rounded-xl p-4">
          <div className="text-xs text-green-400 mb-1">Damage / Point</div>
          <div className="text-3xl font-bold text-green-300">{damage_per_point.toFixed(4)}</div>
        </div>
      </div>
    </div>
  );
